...
export default function IssuesTreeInteractive() {
  ...
  const Leaf = ({ text, color, pruned }: { text: string; color: string; pruned?: boolean }) => (
  ...
  const SubBranch = ({ sub, color }: { sub: SubData; color: string }) => {
    ...
    <ScrollReveal>
      <div className="my-8">
        <h3 style={{ fontWeight: 700, fontSize: '1.1rem', color: '#1E1B4B', marginBottom: '0.25rem' }}>
          Issues Tree with Supporting Hypotheses
        </h3>
        <div style={{ background: '#1E1B4B', color: 'white', borderRadius: 6, padding: '0.6rem 1rem', fontWeight: 700, fontSize: '0.875rem', marginBottom: '0.75rem', textAlign: 'center' }}>
          Root: Low Customer Satisfaction Rating
        </div>
        {branches.map((b) => {
